...
import { PrivacyStatsContext, PrivacyStatsProvider } from '../PrivacyProProvider.js';
...
function PrivacyProConfigured({ parentRef, expansion, data, toggle }) {
    ...
    return (
        <div class={styles.root} ref={parentRef}>
            ...
            />
            {expanded && someCompanies && <PrivacyProBody trackerCompanies={data.trackerCompanies} listAttrs={{ id: WIDGET_ID }} />}
        </div>
    );
}
...
export function PrivacyProBody({ trackerCompanies, listAttrs = {} }) {
    const { t } = useTypedTranslationWith(/** @type {enStrings} */ ({}));
...
export function PrivacyProConsumer() {
    ...
    if (state.status === 'ready') {
        return (
            <PrivacyPro expansion={state.config.expansion} animation={state.config.animation?.kind} data={state.data} toggle={toggle} />
        );
    }
    return null;
}
