...
test.describe('onboarding v4 screenshots', { tag: ['@screenshots'] }, () => {
    test('v4-1-welcome', async ({ page }, workerInfo) => {
        ...
        await onboarding.reducedMotion();
        await onboarding.openPage({ debugState: false, page: 'welcome' });
        await expect(page).toHaveScreenshot('v4-1-welcome.png', { maxDiffPixels });
    });

    test('v4-2-getStarted', async ({ page }, workerInfo) => {
        const onboarding = OnboardingV4Page.create(page, workerInfo);
        onboarding.withInitData({
            stepDefinitions: null,
            order: 'v4',
        ...
        await onboarding.reducedMotion();
        await onboarding.openPage({ debugState: false, page: 'getStarted' });
        await expect(page).toHaveScreenshot('v4-2-getStarted.png', { maxDiffPixels });
    });

    test('v4-3-makeDefaultSingle', async ({ page }, workerInfo) => {
        const onboarding = OnboardingV4Page.create(page, workerInfo);
        onboarding.withInitData({
            stepDefinitions: null,
            order: 'v4',
        ...
        await onboarding.reducedMotion();
        await onboarding.openPage({ debugState: false, page: 'makeDefaultSingle' });
        await expect(page).toHaveScreenshot('v4-3-makeDefaultSingle.png', { maxDiffPixels });
    });

    test('v4-4-systemSettings', async ({ page }, workerInfo) => {
        const onboarding = OnboardingV4Page.create(page, workerInfo);
        onboarding.withInitData({
            stepDefinitions: null,
            order: 'v4',
        ...
        await onboarding.skippedCurrent(); // import
        await page.getByRole('button', { name: 'Next' }).waitFor();
        await expect(page).toHaveScreenshot('v4-4-systemSettings.png', { maxDiffPixels });
    });

    test('v4-5-duckPlayerSingle', async ({ page }, workerInfo) => {
        const onboarding = OnboardingV4Page.create(page, workerInfo);
        onboarding.withInitData({
            stepDefinitions: null,
            order: 'v4',
    ...
    test('v4-6-customize', async ({ page }, workerInfo) => {
        ...
        await page.getByRole('switch', { name: 'Enable Session Restore' }).waitFor();
        await page.getByRole('switch', { name: 'Show Home Button' }).waitFor();
        await expect(page).toHaveScreenshot('v4-6-customize.png', { maxDiffPixels });
    });

    test('v4-7-addressBarMode', async ({ page }, workerInfo) => {
        const onboarding = OnboardingV4Page.create(page, workerInfo);
        onboarding.withInitData({
            stepDefinitions: null,
            order: 'v4',
        ...
        await onboarding.reducedMotion();
        await onboarding.openPage({ debugState: false, page: 'addressBarMode' });
        await expect(page).toHaveScreenshot('v4-7-addressBarMode.png', { maxDiffPixels });
    });
});
