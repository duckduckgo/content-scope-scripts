...
- When implementing Figma designs in special-pages (especially new-tab), map Figma CSS variable names to existing design-system / NTP tokens; verify tokens in `@duckduckgo/design-tokens`, `theme-mappings.css`, or `ntp-theme.css` — do not invent tokens or paste Figma vars that are not defined in the repo; if no exact match, use the closest existing token.
