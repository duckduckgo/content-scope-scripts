...
export default class DuckPlayerFeature extends ContentFeature {
    init (args) {
        // console.log('duck?', JSON.stringify(args, null, 2))
        if (!this.messaging) {
            throw new Error('cannot operate without a messaging backend')
        }
        ...
        if (!overlaysEnabled && overlaySetting.length === 0) {
            if (env.allowedOverlayOrigins.includes(globalThis.location.origin)) {
        ...
        if (!proxyEnabled && proxySetting.length === 0) {
            if (env.allowedProxyOrigins.includes(globalThis.location.origin)) {
                proxyEnabled = true
            }
        }

        if (overlaysEnabled) {
            console.log('✅ overlays enabled')
