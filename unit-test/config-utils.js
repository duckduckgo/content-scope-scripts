...
import { readAll } from '../scripts/pull-remotes.js'
...
describe('Platform Configuration', () => {
    ...
    it('validates current remote configs', async () => {
        const json = {
            contentScope: {},
            userUnprotectedDomains: [],
            userPreferences: {
        ...
        const remotes = await readAll()
        for (const [, value] of Object.entries(remotes)) {
            const withRemote = {
                ...json,
                contentScope: value.remote
            }
            const config = createRuntimeConfiguration(withRemote)
