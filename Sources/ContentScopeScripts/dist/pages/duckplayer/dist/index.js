...
(() => {
  ...
  // pages/duckplayer/app/features/buffering-metrics.js
  var PLAYBACK_EVENT = "duckplayer:playbackEvent";
  var BufferingMetrics = class {
    /**
     * @param {HTMLIFrameElement} iframe
     * @returns {() => void}
     */
    iframeDidLoad(iframe) {
      ...
      const getBufferAhead = () => {
        ...
        for (let i3 = 0; i3 < video.buffered.length; i3++) {
          if (video.buffered.start(i3) <= currentTime && currentTime <= video.buffered.end(i3)) {
            return video.buffered.end(i3) - currentTime;
          }
        }
        return 0;
      };
