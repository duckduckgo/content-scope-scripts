...
(() => {
  ...
  // shared/metrics-reporter.js
  var EXCEPTION_KIND_GENERIC_ERROR = "Error";
  var EXCEPTION_KIND_INIT_ERROR = "InitError";
  ...
  async function init(messaging2, telemetry2, baseEnvironment2) {
    ...
    try {
      ...
      messaging2.metrics.reportException({ message: e3.message, kind: EXCEPTION_KIND_INIT_ERROR });
    }
    const didCatch = (error) => {
      const message = error?.message;
      messaging2.metrics.reportExceptionWithError(error?.error);
      messaging2.reportPageException({ message: message || "unknown error" });
    };
  ...
  var DuckplayerPage = class {
    ...
    constructor(messaging2, injectName) {
      ...
      this.injectName = injectName;
      this.metrics = new MetricsReporter(messaging2);
    }
    /**
    ...
    async initialSetup() {
      ...
      try {
        ...
        this.metrics.reportException({ message: e3?.message, kind: EXCEPTION_KIND_MESSAGING_ERROR });
        throw e3;
      }
    }
    /**
  ...
  init(duckplayerPage, telemetry, baseEnvironment).catch((e3) => {
    console.error(e3);
    duckplayerPage.metrics.reportExceptionWithError(e3);
    const message = typeof e3?.message === "string" ? e3.message : "unknown error";
