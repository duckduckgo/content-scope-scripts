...
`;
  ...
  // src/features/broker-protection/captcha-services/providers/botshield.js
  init_define_import_meta_trackerLookup();
  var botShieldConfig = {
    type: "botShield",
  ...
  var getDocument = (element) => element.ownerDocument ?? document;
  var getTokenInputs = (root) => {
    return botShieldConfig.tokenSelectors.reduce((accumulator, selector) => {
  ...
  var BotShieldProvider = class {
    ...
    injectToken(captchaContainerElement, token) {
      ...
      const failure = tokenInputs.map((captchaInputElement) => injectTokenIntoElement({ captchaInputElement, token })).find((response) => PirError.isError(response));
      if (failure) {
        return failure;
      }
      return PirSuccess.create({ injected: true });
