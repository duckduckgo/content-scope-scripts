...
`;
  ...
  var AddressFullExtractor = class {
     ...
     */
    extract(strs, extractorParams) {
      return strs.map((str) => str.replace("\n", " ")).map((str) => stringToList(str, extractorParams.separator)).flat().map((str) => import_parse_address.default.parseLocation(str) || {}).filter(
        /**
  ...
  var DDGCtlLoginButton = class {
    ...
    constructor(params) {
      ...
      const shadow = __privateGet(this, _element).attachShadow({
        mode: this.params.devMode ? "open" : "closed"
      });
      const style = document.createElement("style");
