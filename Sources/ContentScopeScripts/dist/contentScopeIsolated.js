...
`;
  ...
  function extractAddressFull(select2, root, spec) {
    return selectStrings(select2, root, spec).map((str) => str.replace("\n", " ")).flatMap((str) => stringToList(str, spec.separator)).map((str) => import_parse_address.default.parseLocation(str) || {}).filter((parsed) => Boolean(parsed?.city)).map((addr) => {
      const street = [addr.number, addr.prefix, addr.street, addr.type, addr.suffix, addr.sec_unit_type, addr.sec_unit_num].filter(Boolean).join(" ");
      return {
        city: addr.city,
  ...
  var KNOWN_PROFILE_FIELDS = new Set(Object.keys(extractors));
  function createProfile(select2, root, profileSpec) {
    ...
    for (const [field, fieldSpec] of Object.entries(profileSpec)) {
      ...
      if (extractField) {
        ...
        output[field] = extractDefault(select2, root, fieldSpec);
      }
    }
    return output;
  }
  function extractDefault(select2, root, spec) {
  ...
  function fillMany(root, elements, data2) {
    ...
    for (const element of elements) {
      ...
      if (element.type === "$file_id$") {
        ...
        const present = Object.prototype.hasOwnProperty.call(data2, element.type);
        const value = present ? data2[element.type] : void 0;
        if (!present || !value) {
          if (optional) {
            continue;
          }
