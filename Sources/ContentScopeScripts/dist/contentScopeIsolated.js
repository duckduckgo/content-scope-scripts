...
(() => {
  ...
  var ConfigFeature = class {
     ...
     * @returns {boolean}
     */
    getFeatureSettingEnabled(featureKeyName, defaultState2, featureName) {
      const result = this.getFeatureSetting(featureKeyName, featureName) || defaultState2;
...
`;
  ...
  var DEFAULT_RUN_CONDITIONS = (
    /** @type {import('../../config-feature.js').ConditionBlockOrArray} */
    [
      {
        context: { top: true }
      }
  ...
  function normalizeDetector(config2) {
    const breakageRunConditions = (
      /** @type {import('../../config-feature.js').ConditionBlockOrArray} */
      config2.triggers?.breakageReport?.runConditions ?? DEFAULT_RUN_CONDITIONS
    );
    const autoRunConditions = (
      /** @type {import('../../config-feature.js').ConditionBlockOrArray} */
      config2.triggers?.auto?.runConditions ?? DEFAULT_RUN_CONDITIONS
    );
    const actions = {
      breakageReportData: { state: defaultState(config2.actions?.breakageReportData?.state, "enabled") }
    };
    const rawActions = (
    ...
    return {
      state: defaultState(config2.state, "enabled"),
      match: config2.match,
      triggers: {
        breakageReport: {
          state: defaultState(config2.triggers?.breakageReport?.state, "enabled"),
          runConditions: breakageRunConditions
        },
        auto: {
          state: defaultState(config2.triggers?.auto?.state, "disabled"),
