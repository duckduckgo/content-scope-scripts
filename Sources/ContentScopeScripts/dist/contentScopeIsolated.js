  ...
  var MessageBridge = class extends ContentFeature {
    ...
    init(args) {
      ...
      accept(InstallProxy, (install) => {
        const config2 = args.messagingConfig;
        if (!config2) return this.log.info("not installing proxy, because messagingConfig was absent");
        this.installProxyFor(install, config2, reply);
      });
    ...
    proxySubscription(subscription, reply) {
      ...
      const unsubscribe = proxy.subscribe(subscriptionName, (data2) => {
        const params = data2 !== null && typeof data2 === "object" && !Array.isArray(data2) ? data2 : void 0;
