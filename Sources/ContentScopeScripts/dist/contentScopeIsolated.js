...
(() => {
  ...
  function featuresForFrame(featureNames, iosBlankSubframe) {
    if (!iosBlankSubframe) return featureNames;
    return featureNames.filter((name) => iosBlankSubframeFeatures.includes(
      /** @type {FeatureName} */
      name
    ));
  }
...
`;
  ...
  function load(args) {
    ...
    const enabledFeatureNames = isGloballyDisabled(args) ? platformSpecificFeatures : args.site.enabledFeatures || bundledFeatureNames;
