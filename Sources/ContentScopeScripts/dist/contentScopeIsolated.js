...
/*
 * Buffering spinner. Reproduces YouTube's own Material "linspin" indeterminate
 * spinner (four nested animations: linear outer spin, eased rotator, and two
 * arc halves that sweep) so it reads as the player's own buffering UI rather
 * than a foreign loader. Hidden until \`.spinning\` is set via showSpinner().
 */
.ddg-vpo-spinner {
...
.ddg-vpo-spinner__container {
    ...
    width: 100%;
    height: 100%;
    animation: ddg-vpo-linspin 1.56824s linear infinite;
}
...
`;
  ...
  var SPINNER_GIVE_UP_MS = 6e4;
  var VIDEO_SWAP_POLL_MS = 500;
  var OVERLAY_TAG_NAMES = [
  ...
  var VideoOverlay = class {
    ...
    constructor({ userValues, settings, environment, messages, ui }) {
      ...
      __publicField(this, "posterSrc", null);
    ...
    showBufferingFeedbackUntilFirstFrame() {
      ...
      this.sideEffects.add("holding poster + spinner until first video frame", () => {
        ...
        const onProgress = () => {
          if (video.currentTime > 0 && !video.paused) remove2();
        };
        const onFirstFrame = () => remove2();
        const listen = (el) => {
