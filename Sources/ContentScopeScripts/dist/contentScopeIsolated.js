...
(function () {
     ...
     */
    const safeCall = (fn, { errorMessage } = {}) => {
        try {
            return fn();
        } catch (e) {
     ...
     * @returns {string|null} - The parameter value or null
     */
    const getUrlHashParameter = (url, param) => {
        if (!url || false) {
            return null;
        }
     ...
     * @throws {Error}
     */
    const getSiteKeyFromSearchParam = ({ captchaElement, siteKeyAttrName }) => {
        if (!captchaElement) {
     ...
     */
    const injectTokenIntoElement = ({ root, elementName, token }) => {
     ...
     */
    const stringifyFunction = ({ functionName, functionBody, args }) => {
        return safeCall(
            () => `;(function(args) {
    ...
    class ReCaptchaEnterpriseProvider extends ReCaptchaV2Provider {
        getType() {
    ...
    class CloudFlareTurnstileProvider extends CaptchaProvider {
        getType() {
     ...
     * @throws {Error}
     */
    const getCaptchaProvider = (captchaDiv, captchaType) => {
     ...
     * @throws {Error}
     */
    const getCaptchaSolveProvider = (root, captchaType) => {
        const captchaProvider = captchaFactory.getProviderByType(captchaType);
        if (!captchaProvider) {
     ...
     * @throws {Error}
     */
    const getSupportingCodeToInject = (captchaType) => {
        const captchaProvider = captchaFactory.getProviderByType(captchaType);
        if (!captchaProvider) {
     ...
     * @return {import('../types.js').ActionResponse}
     */
    const getCaptchaInfo = (action, root = document) => {
     ...
     * @return {import('../types.js').ActionResponse}
     */
    const solveCaptcha = (action, token, root = document) => {
     ...
     * @return {import('../types.js').ActionResponse}
     */
    const navigate = (action, userData) => {
     ...
     */
    const resolveActionHandlers = ({ useNewActionHandlers }) => {
        return {
            extract,
            fillForm,
            click,
