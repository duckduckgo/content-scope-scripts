...
(() => {
  ...
  init_define_import_meta_trackerLookup();
  var EXCEPTION_KIND_GENERIC_ERROR = "Error";
  var EXCEPTION_KIND_INITIAL_SETUP_ERROR = "InitialSetupError";
  var EXCEPTION_KIND_MESSAGING_ERROR = "MessagingError";
  var _MetricsReporter = class _MetricsReporter {
    /**
    ...
    _sendMetric(metricEvent) {
      ...
      this.messaging.notify(_MetricsReporter.MESSAGE_ID, metricEvent);
    }
    /**
    ...
    _createExceptionMetric(params) {
      const message = params?.message && typeof params.message === "string" ? params.message : _MetricsReporter.DEFAULT_EXCEPTION_MESSAGE;
      const kind = params?.kind && typeof params.kind === "string" ? params.kind : EXCEPTION_KIND_GENERIC_ERROR;
      return {
        metricName: _MetricsReporter.METRIC_NAME_EXCEPTION,
        params: {
          message,
          kind
        }
  ...
  var MetricsReporter = _MetricsReporter;
  ...
  var DuckPlayerOverlayMessages = class {
    ...
    async initialSetup() {
      ...
      try {
        return await this.messaging.request(MSG_NAME_INITIAL_SETUP);
      } catch (e) {
    ...
    async setUserValues(userValues) {
      try {
        return await this.messaging.request(MSG_NAME_SET_VALUES, userValues);
      } catch (e) {
        this.metrics.reportException({ message: e?.message, kind: EXCEPTION_KIND_MESSAGING_ERROR });
        throw e;
      }
    ...
    async getUserValues() {
      try {
        return await this.messaging.request(MSG_NAME_READ_VALUES, {});
      } catch (e) {
        this.metrics.reportException({ message: e?.message, kind: EXCEPTION_KIND_MESSAGING_ERROR });
        throw e;
      }
  ...
  // src/features/duckplayer/components/ddg-video-overlay.js
  var EXCEPTION_KIND_VIDEO_OVERLAY_ERROR = "VideoOverlayError";
...
`;
  ...
  var VideoOverlay = class {
    ...
    appendMobileOverlay(targetElement, params) {
      ...
      this.sideEffects.add(`appending ${DDGVideoOverlayMobile.CUSTOM_TAG_NAME} to the page`, () => {
        ...
        elem.addEventListener(DDGVideoOverlayMobile.OPT_OUT, (e) => {
          return this.mobileOptOut(e.detail.remember).catch((e2) => {
            console.error(e2);
            this.messages.metrics.reportExceptionWithError(e2);
          });
        });
        elem.addEventListener(DDGVideoOverlayMobile.OPT_IN, (e) => {
          return this.mobileOptIn(e.detail.remember, params).catch((e2) => {
            console.error(e2);
            this.messages.metrics.reportExceptionWithError(e2);
          });
        });
    ...
    userOptOut(remember, params) {
      if (remember) {
        ...
        }).then(() => this.watchForVideoBeingAdded({ ignoreCache: true, via: "userOptOut" })).catch((e) => {
          console.error(e);
          this.messages.metrics.reportExceptionWithError(e);
        });
      } else {
  ...
  var DuckPlayerFeature = class extends ContentFeature {
    init(args) {
      ...
      try {
        ...
        if (overlaysEnabled) {
          initOverlays(overlaySettings.youtube, env, comms).catch((e) => {
            console.error(e);
  ...
  var DuckPlayerNativeMessages = class {
    ...
    async initialSetup() {
      try {
        return await this.messaging.request(MSG_NAME_INITIAL_SETUP2);
      } catch (e) {
        this.metrics.reportException({ message: e?.message, kind: EXCEPTION_KIND_MESSAGING_ERROR });
        throw e;
      }
  ...
  var DuckPlayerNativeFeature = class extends ContentFeature {
    ...
    init(args) {
      ...
      try {
        ...
        this.initDuckPlayerNative(messages, selectors, env).catch((e) => {
          console.error(e);
          metrics.reportExceptionWithError(e);
        });
      } catch (e) {
