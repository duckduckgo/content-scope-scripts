...
(() => {
  ...
  var DuckPlayerOverlayMessages = class {
    ...
    onUserValuesChanged(cb) {
      return this.messaging.subscribe(
        "onUserValuesChanged",
    ...
    init() {
      this.sideEffects.add("showing overlays on hover", () => {
        ...
        const mouseOverHandler = (e) => {
          ...
          if (e.target === hoverElement || hoverElement?.contains(
            /** @type {Node | null} */
            e.target
          )) {
          ...
          const matched = target && selectors.allowedEventTargets.find((css) => target.matches(css));
          if (matched) {
    ...
    init() {
      this.sideEffects.add("intercepting clicks", () => {
        ...
        const clickHandler = (e) => {
          ...
          if (e.target === elementInStack || elementInStack?.contains(
            /** @type {Node | null} */
            e.target
          )) {
          ...
          const matched = clickTarget && selectors.allowedEventTargets.find((css) => clickTarget.matches(css));
          if (matched) {
            block(validLink);
          }
        };
...
`;
  ...
  function getErrorType(windowObject, signInRequiredSelector, logger) {
    const currentWindow = (
      /** @type {Window & typeof globalThis & { ytcfg: { get(key: string): any } }} */
      windowObject
    );
  ...
  function buildUrl(action, userData) {
    ...
    return new SuccessResponse({
      actionID: action.id,
      actionType: (
        /** @type {import('../types.js').PirAction['actionType']} */
        action.actionType
      ),
  ...
  function replaceTemplatedUrl(action, userData) {
    ...
    return transformUrl(
      /** @type {import('./build-url-transforms.js').BuildUrlAction} */
      action,
      userData
    );
  }
          ...
          unblockClickToLoadContent({ entity: this.entity, action, isLogin, isSurrogateLogin }).then((response) => {
            ...
            switch (this.clickAction.type) {
              case "iFrame":
                ...
                break;
              case "youtube-video":
                onError = this.adjustYouTubeVideoElement(
  ...
  function resizeElementToMatch(sourceElement, targetElement) {
    ...
    for (const key of stylesToCopy) {
      targetElement.style[key] = /** @type {any} */
      computedStyle[key];
    }
  ...
  var _ProxyResponse = class _ProxyResponse {
    ...
    static create(params) {
      ...
      return new _ProxyResponse({
        ...
        error: (
          /** @type {import("@duckduckgo/messaging").MessageError | undefined} */
          params.error
        ),
        id: params.id
      });
  ...
  function getFaviconList() {
    ...
    return Array.from(elements).map((link) => {
      ...
      const rel = htmlLink.getAttribute("rel") || "";
      const type = htmlLink.type || "";
