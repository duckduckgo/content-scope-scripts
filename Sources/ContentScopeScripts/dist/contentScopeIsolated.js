...
`;
  ...
  function extractAddressFull(select2, root, spec) {
    return selectStrings(select2, root, spec).flatMap((str) => fullAddressStrings(str, spec.separator)).map((fullAddress) => ({
      fullAddress,
      parsed: (
        /** @type {ParsedAddress} */
        import_parse_address.default.parseLocation(fullAddress.replace(/\s+/g, " ")) || {}
      )
    })).filter(({ parsed }) => Boolean(parsed.city)).map(({ fullAddress, parsed }) => {
      return {
        ...parsed,
        city: (
          /** @type {string} */
  ...
  function fillMany(root, elements, data2) {
    ...
    for (const element of elements) {
      ...
      if (element.type === "$file_id$") {
        ...
        results.push(setValueForInput(inputElem, data2.street || generateStreetAddress()));
      } else if (element.type === "zipCode") {
        results.push(setValueForInput(inputElem, data2.zipCode || generateZipCode()));
