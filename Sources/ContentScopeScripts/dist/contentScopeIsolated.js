...
`;
  ...
  var urlChangesStarted = false;
  var URL_CHANGED_BRIDGE_EVENT = "URL_CHANGED";
  ...
  function broadcastURLChangesToIsolatedWorld(messageSecret2) {
    registerForURLChanges((navigationType) => {
      const event = new CustomEvent2(`${URL_CHANGED_BRIDGE_EVENT}-${messageSecret2}`, {
        detail: { navigationType }
      });
      dispatchEvent?.(event);
    });
  }
  function listenForBroadcastedURLChanges(messageSecret2) {
    urlChangesStarted = true;
    addEventListener?.(`${URL_CHANGED_BRIDGE_EVENT}-${messageSecret2}`, (event) => {
      handleURLChange(
        /** @type {CustomEvent<{navigationType: NavigationType}>} */
        event.detail.navigationType
      );
    });
  }
  ...
  async function init(args) {
    ...
    if (args.messageSecret && !hasNavigationAPI()) {
      const isIsolatedWorld = "apple-isolated"?.endsWith("-isolated");
      if (isIsolatedWorld) {
