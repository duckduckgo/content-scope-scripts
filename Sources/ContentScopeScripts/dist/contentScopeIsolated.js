...
`;
  ...
  var DuckWidget = class {
    ...
    constructor(widgetData, originalElement, entity, platform) {
      this.clickAction = { type: "", targetURL: "", ...widgetData.clickAction };
  ...
  function makeModal(entity, acceptFunction, ...acceptFunctionParams) {
    const data2 = entityData[entity];
    if (!data2) return;
    const icon = data2.modalIcon;
    ...
    const allowButton = makeButton(data2.modalAcceptText ?? "", "lightMode");
    ...
    const rejectButton = makeButton(data2.modalRejectText ?? "", "cancelMode");
