...
(() => {
  ...
  __export(captured_globals_exports, {
    ...
    createXPathExpression: () => createXPathExpression,
    ...
    encrypt: () => encrypt,
    evaluateXPathExpression: () => evaluateXPathExpression,
    exportKey: () => exportKey,
  ...
  var parseFloat2 = globalThis.parseFloat;
  ...
  function stringSlice(string, start) {
  ...
  function elementQuerySelector(element, selectors) {
    return ReflectApply(capturedElementQuerySelector, element, [selectors]);
  }
  function elementQuerySelectorAll(element, selectors) {
...
`;
  ...
  var VALID_NAME = new RegExp2("^[a-zA-Z][a-zA-Z0-9_]*$");
  ...
  function parseDetectors(detectorsConfig) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      for (let j2 = 0; j2 < groupEntries.length; j2++) {
        const detectorId = groupEntries[j2][0];
        const detectorConfig = groupEntries[j2][1];
  ...
  function isVisible2(element) {
    ...
    return rectWidth(rect) > 0.5 && rectHeight(rect) > 0.5 && cssPropertyValue(style, "display") !== "none" && cssPropertyValue(style, "visibility") !== "hidden" && parseFloat2(cssPropertyValue(style, "opacity")) > 0.05;
  }
  ...
  function resolveXPathConfig(config2) {
    ...
    const chunkTail = config2?.chunkTail ?? mathFloor(chunkSize / CHUNK_TAIL_RATIO);
  ...
  function xpathMatches(pattern, expression, { chunkSize, chunkTail }) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      const text2 = node && nodeTextContent(node) || "";
      buffer += text2;
  ...
  function evaluateSingleTextCondition(condition2) {
    ...
    const patternComb = new RegExp2(join(patterns, "|"), "i");
    for (let i = 0; i < selectors.length; i++) {
      const elements = documentQuerySelectorAll(at(selectors, i));
      for (let j2 = 0; j2 < nodeListLength(elements); j2++) {
        if (regExpTest(patternComb, nodeTextContent(nodeListItem(elements, j2)) || "")) {
          return true;
        }
      }
    }
  ...
  function evaluateSingleElementCondition(config2) {
    ...
    const selectors = asArray(config2.selector);
    for (let i = 0; i < selectors.length; i++) {
      ...
      for (let j2 = 0; j2 < nodeListLength(elements); j2++) {
        const element = nodeListItem(elements, j2);
        if (visibility === "visible" && isVisible2(element)) {
          return true;
        }
        if (visibility === "hidden" && !isVisible2(element)) {
          return true;
        }
  ...
  function evaluateSingleMatchCondition(condition2) {
    if (!evaluateNode(condition2.text, evaluateSingleTextCondition)) {
      return false;
    }
    if (!evaluateNode(condition2.element, evaluateSingleElementCondition)) {
      return false;
    }
    return true;
  }
  ...
  // src/features/text-selection.js
  init_define_import_meta_trackerLookup();
  var TextSelection = class extends ContentFeature {
    constructor() {
      super(...arguments);
      /** @type {boolean | null} */
      __publicField(this, "_lastHasSelection", null);
      /** @type {number | null} */
      __publicField(this, "_clearTimer", null);
      /** @type {number | null} */
      __publicField(this, "_claimTimer", null);
      /** @type {string} */
      __publicField(this, "_snapshot", "");
      /** @type {number} */
      __publicField(this, "_snapshotTimestamp", 0);
    }
    init() {
    ...
    _selectionChanged(event) {
      ...
      const text2 = this._selectionText();
      const hasSelection = text2.trim().length > 0;
      const canClaim = this._canClaim();
      if (hasSelection) {
        this._cancelPendingClear();
        if (!this._publish(text2, canClaim, eventTimestamp)) this._scheduleClaimRetry(eventTimestamp);
        return;
      }
      if (this._clearTimer !== null) return;
      this._clearTimer = window.setTimeout(() => {
        this._clearTimer = null;
        this._publish(this._selectionText(), void 0, eventTimestamp);
      }, 100);
    }
    ...
    _scheduleClaimRetry(eventTimestamp) {
      this._claimTimer = window.setTimeout(() => {
        ...
        this._publish(text2, canClaim, eventTimestamp);
      }, 0);
    }
    _cancelPendingClaim() {
     ...
     * @returns {boolean}
     */
    _publish(text2, canClaim = this._canClaim(), eventTimestamp = this._now()) {
    ...
    _pageShown(event) {
      ...
      this._publish(this._selectionText(), void 0, this._eventTimestamp(event));
    }
  };
  ...
  var ddg_platformFeatures_default = {
    ...
    ddg_feature_textSelection: TextSelection
  };
