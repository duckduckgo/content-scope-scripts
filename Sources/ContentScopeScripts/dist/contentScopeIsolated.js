  ...
  async function execute(action, inputData, root = document) {
    try {
      switch (action.actionType) {
        ...
        case "getCaptchaInfo":
          return await getCaptchaInfo2(action, data(action, inputData, "userProfile"), root);
        case "solveCaptcha":
          return solveCaptcha2(action, data(action, inputData, "token"), inputData?.userProfile ?? null, root);
        case "condition":
