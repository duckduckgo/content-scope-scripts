...
| `buttonCopy`             | Optional `{install, remove, unavailable, unavailableDescription}` override — the copy hot-fix channel. Bundled locale strings are the default source. A verdict without its copy stays hidden.                                          |
