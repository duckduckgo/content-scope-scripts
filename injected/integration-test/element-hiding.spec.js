...
test.describe('Element Hiding Integration Tests', () => {
    test('Basic element hiding with simple hide rules', async ({ page }, testInfo) => {
        ...
        await collector.load('/element-hiding/pages/basic-hiding.html', 
    ...
    test('Hide empty elements only', async ({ page }, testInfo) => {
        const collector = ResultsCollector.create(page, testInfo.project.use);
        await collector.load('/element-hiding/pages/empty-elements.html',
    ...
    test('Modify element attributes', async ({ page }, testInfo) => {
        const collector = ResultsCollector.create(page, testInfo.project.use);
        await collector.load('/element-hiding/pages/modify-attributes.html',
    ...
    test('Modify element styles', async ({ page }, testInfo) => {
        const collector = ResultsCollector.create(page, testInfo.project.use);
        await collector.load('/element-hiding/pages/modify-styles.html',
    ...
    test('Performance: Element hiding rules apply quickly', async ({ page }, testInfo) => {
        ...
        const adBanner = await page.locator('.ad-banner').first();
        const isHidden = await adBanner.evaluate(el => {
            const style = window.getComputedStyle(el);
            return style.display === 'none' || el.hidden === true;
        });
    ...
    test('Forgiving selectors handle invalid CSS gracefully', async ({ page }, testInfo) => {
        // Create a test config with potentially problematic selectors
        const testConfig = {
            "version": 1,
            ...
            "features": {
                "elementHiding": {
                    ...
                    "rules": [
                        ...
                        {
                            "selector": ".invalid[syntax:here",
                            "type": "hide"
                        },
                        {
                            "selector": "#another-valid",
                            "type": "hide"
                        }
                    ]
                }
        ...
        const collector = ResultsCollector.create(page, testInfo.project.use);
        await collector.load('/element-hiding/pages/basic-hiding.html', testConfig);
