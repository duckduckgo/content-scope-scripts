...
test.describe('Device Enumeration Feature', () => {
    ...
    test.describe('enabled feature', () => {
        ...
        test('shims every brand-checked prototype method, but preserves explicit toJSON semantics', async ({ page }) => {
            await gotoAndWait(page, '/blank.html', {
                site: {
                    enabledFeatures: ['webCompat'],
                },
            ...
            // Plant a synthetic future-method on InputDeviceInfo.prototype *before* enumerateDevices
            // is called, so the generic method shim has to discover and cover it.
            await page.evaluate(() => {
                Object.defineProperty(InputDeviceInfo.prototype, 'futurePrototypeMethod', {
                    value: function () {
            ...
            const result = await page.evaluate(async () => {
                const devices = await navigator.mediaDevices.enumerateDevices();
                const audioInput = /** @type {any} */ (devices.find((d) => d.kind === 'audioinput'));
                const audioOutput = /** @type {any} */ (devices.find((d) => d.kind === 'audiooutput'));

                let futureMethodThrew = false;
                let futureMethodResult;
                try {
                    futureMethodResult = audioInput.futurePrototypeMethod();
                } catch {
                    futureMethodThrew = true;
                }

                return {
                    inputFutureMethodThrew: futureMethodThrew,
                    inputFutureMethodResult: futureMethodResult,
                    inputFutureMethodName: audioInput.futurePrototypeMethod.name,
                    inputFutureMethodToString: audioInput.futurePrototypeMethod.toString(),
                    inputToJSON: audioInput.toJSON(),
                    outputToJSON: audioOutput.toJSON(),
                };
            });

            expect(result.inputFutureMethodThrew).toEqual(false);
            expect(result.inputFutureMethodResult).toEqual({});
            expect(result.inputFutureMethodName).toEqual('futurePrototypeMethod');
            expect(result.inputFutureMethodToString).toEqual('function futurePrototypeMethod() { [native code] }');
            // toJSON has explicit own semantics on the instance and must NOT be replaced by the
            // generic {} shim.
            expect(result.inputToJSON).toEqual({
                deviceId: 'default',
                kind: 'audioinput',
                label: '',
                groupId: 'default-group',
            });
            expect(result.outputToJSON).toEqual({
                deviceId: 'default',
                kind: 'audiooutput',
                label: '',
                groupId: 'default-group',
