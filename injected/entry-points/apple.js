...
function initCode() {
    ...
    const platformSpecific = injectName === 'apple' ? [...platformSpecificFeatures, 'printing'] : platformSpecificFeatures;
    const processedConfig = processConfig(config, userUnprotectedDomains, userPreferences, platformSpecific);

    const handlerNames = [];
    if (injectName === 'apple-isolated') {
