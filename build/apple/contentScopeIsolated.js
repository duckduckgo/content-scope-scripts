...
`;
  ...
  var _InterferenceMonitor = class _InterferenceMonitor {
    ...
    addListener(config2, onDetectionChange) {
      const id = Symbol();
      this.listeners.set(id, { types: config2.types, callback: onDetectionChange });
      if (!this.observer) {
        this.start();
      }
      return () => {
  ...
  function detectWebInterference(config2) {
    if (config2.observeDOMChanges) {
      const monitor2 = InterferenceMonitor.getInstance({ detectFn: detectTypes });
      return monitor2.addListener(config2, config2.onDetectionChange);
    }
    return detectTypes(config2.types);
  }
