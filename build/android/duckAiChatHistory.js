...
(() => {
  ...
  var _DuckAiChatHistory = class _DuckAiChatHistory extends ContentFeature {
    ...
    async getChats(params) {
      ...
      const maxChats = typeof safeParams.max_chats === "number" ? safeParams.max_chats : _DuckAiChatHistory.DEFAULT_MAX_CHATS;
      const since = typeof safeParams.since === "number" ? safeParams.since : void 0;
      try {
        ...
        this.notify("duckAiChatsResult", {
          success: false,
          error: error && typeof error === "object" && "message" in error && typeof error.message === "string" ? error.message : "Unknown error occurred",
          pinnedChats: [],
          chats: [],
          timestamp: Date.now()
        });
