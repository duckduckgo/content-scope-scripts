...
(() => {
  ...
  // src/features/broker-protection.js
  function ActionExecutorMixin(ContentFeatureClass) {
    return class Mixin extends ContentFeatureClass {
       ...
       */
      async processActionAndNotify(action, data2, retryConfig) {
        try {
          if (!action) {
            return this.messaging.notify("actionError", { error: "No action found." });
          }
          const { results, exceptions } = await this.exec(action, data2, retryConfig);
          if (results) {
       ...
       */
      async exec(action, data2, retryConfig) {
        ...
        if (result) {
          if ("success" in result && Array.isArray(result.success.next)) {
            ...
            for (const nextAction of result.success.next) {
              const { results: subResults, exceptions: subExceptions } = await this.exec(nextAction, data2, retryConfig);
  ...
  var BrokerProtection = class extends ActionExecutorMixin(ContentFeature) {
    init() {
      this.messaging.subscribe("onActionReceived", async (params) => {
        const { action, data: data2 } = params.state;
        return await this.processActionAndNotify(action, data2, this.retryConfigFor(action));
      });
    }
    /**
