...
(() => {
  ...
  // src/content-feature.js
  var _bundledConfig, _trackerLookup, _documentOriginIsTracker, _bundledfeatureSettings, _messaging, _isDebugFlagSet, _args;
  var ContentFeature = class {
  ...
  var WebCompat = class extends ContentFeature {
    ...
    mediaSessionFix() {
      try {
        if (window.navigator.mediaSession && false) {
          return;
        }
    ...
    presentationFix() {
      try {
        if (window.navigator.presentation && false) {
          return;
        }
  ...
  function load(args) {
    ...
    const featureNames = true ? platformSupport["integration"] : [];
  ...
  function generateConfig() {
    ...
    const trackerLookup2 = define_import_meta_trackerLookup_default;
    return {
      debug: false,
