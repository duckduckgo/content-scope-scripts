...
Normal releases are automated ([Release workflow](https://github.com/duckduckgo/content-scope-scripts/actions/workflows/build.yml)): they copy `main` onto the `releases` branch, build, commit the artifacts and tag. A hotfix does the same thing, but starts from an existing release tag instead of `main`, so only the fix ships. Because built code is checked in, there is more to it than tagging a commit.
...
Use the [Hotfix Release workflow](https://github.com/duckduckgo/content-scope-scripts/actions/workflows/hotfix.yml). Give it the release tag you are fixing and the commits to cherry-pick, and it does everything up to a **draft** release: branches from the tag, cherry-picks, builds, checks in the artifacts, pushes `hotfix/<version>` and pushes the changelog anchor. Review the compare link in the job summary, then publish the draft yourself.

It refuses to run if a commit is not already on `main`, if the target tag exists, or if a cherry-pick conflicts. In those cases, and for anything else that needs judgement, fall back to [doing it by hand](#doing-it-by-hand). Set `dry_run` to build and verify without pushing anything.

> [!WARNING]
> The workflow asks for the release to be a draft and sets `make_latest: false`, but GitHub still shows a **Set as the latest release** tickbox when you publish. Confirm it is unticked, unless this genuinely is the latest release.
...
The workflow mirrors the steps below, so this is both the fallback and the explanation of what it does.

### 1. Create and build the hotfix branch

Work in a **clean clone or worktree**, because step 4 force-adds files and stray local output (`coverage/`, `test-results/`, `docs/`, `.env`) would be swept in.
...
- **Keep `hotfix/{release-tag+1}` around.** The tag is what actually pins the commits, so publishing the release first is the step that protects them. Do not delete the branch before the tag exists, and keeping it afterwards matches existing practice (`hotfix/4.21.2`, `hotfix/4.22.5` and `hotfix/12.4.1` are all still present) and makes a follow-up hotfix on the same line straightforward.
